  ...
  if (!profile) {
    return (
      <div className="kh-panel p-5">
        ...
        <p className="mt-2 text-sm text-slate-600">Create or select a profile in Setup before running a Session.</p>
      </div>
    );
  }
  ...
  return (
    <div className="kh-panel p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <StatusBadge label={profile.model_ready ? "Ready" : "Not Ready"} tone={profile.model_ready ? "good" : "warn"} />
      </div>

      <dl className="mt-4 space-y-3 text-xs text-slate-600">
        <div>
          <dt className="font-semibold text-slate-500">User Checkpoint</dt>
          <dd className="mt-1 break-all rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5">{profile.user_model_path ?? "No checkpoint yet"}</dd>
        </div>
      </dl>
    </div>
  );
